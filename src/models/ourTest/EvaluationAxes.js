...
AxesDimension.tagDimension = []

AxesDimension.add = async function (dimension) {
    AxesDimension.allAxesByDimension.sector = FilterBySector.allFilterBySector.sector;
    AxesDimension.allAxesByDimension.sectorId = FilterBySector.allFilterBySector.sectorId;
    AxesDimension.allAxesByDimension.dimension = dimension;
}


AxesDimension.tagDimension = function (data) {
    let object = ""; let idDimension = [];
    let tagDimension = []; let j = 0;
    let dimensions = []; let idFormulary = [];
    data.map(element => {
    ...
    AxesDimension.axesDimension = dimensions
    AxesDimension.tagDimension = tagDimension
    return [tagDimension, idDimension, dimensions]
}

AxesDimension.arrayToObject = async function (data) {
    let [tagDimension, idDimension, dimensions] = await AxesDimension.tagDimension(data)
...
AxesDimension.pullDB = async function () {
    AxesDimension.allAxesByDimension = {}
    AxesDimension.axesDimension.length = 0
    AxesDimension.tagDimension.length = 0
